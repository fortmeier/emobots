...
def mood_analysis(name, _chat_messages, _mood_analyis_system_prompt):
    ...
    mood_analysis_completion = openai.ChatCompletion.create(
        model="gpt-3.5-turbo", messages=mood_analysis_messages, temperature=0.3
    )
    mood_analysis_response = mood_analysis_completion["choices"][0]["message"][
        "content"
    ]
