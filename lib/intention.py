...
def intention_analysis(name, _chat_messages, _intention_analyis_system_prompt):
    ...
    intention_analysis_completion = openai.ChatCompletion.create(
        model="gpt-3.5-turbo", messages=intention_analysis_messages, temperature=0
    )
    intention_analysis_response = intention_analysis_completion["choices"][0][
        "message"
    ]["content"]
